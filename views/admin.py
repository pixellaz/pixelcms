...
def manage_general_settings(request):
	...
	#If there is no settings definded
	if request.method != 'POST':
		try:
			...
			form = form_class()
			context = {
			    'title': 'Add new site settings',
			    'form': form,
			}
			return render_to_response(_lookup_template('manage_site_settings'), context,
		                          		context_instance=RequestContext(request))

	#If settings is already defined, edit settings
	if request.method == 'POST':
		form = form_class(request.POST)
		if form.is_valid():
			settings = GeneralSettings(**form.cleaned_data)
