...
MARKUP_LANGUAGE = getattr(settings, 'PIXELCMS_MARKUP_LANGUAGE', None)
...
class PageType(Document):
    title = StringField(required=True)
    slug = StringField(required=True, regex='[A-z0-9_-]+', unique=True)
    author = ReferenceField(User)
    creation_date = DateTimeField(required=True, default=datetime.now)
    published = BooleanField(default=True)
    publish_date = DateTimeField(required=True, default=datetime.now)
    expiry_date = DateTimeField(required=False, default=None)
    nav_display = BooleanField(default=False)
    content = StringField(required=True)
    rendered_content = StringField(required=True)

    meta = {
        'indexes': ['slug', '-publish_date'],
    }
    ...
    def get_absolute_url(self):
		#date = self.publish_date.strftime('%Y/%b/%d').lower()
		#return ('page-detail', (date, self.slug))
    ...
    class AdminForm(forms.Form):
        ...
        expiry_date = forms.DateTimeField(
            ...
            required=False)
        expiry_time = forms.TimeField(required=False)
        nav_display = forms.BooleanField(required=False, initial=False)
